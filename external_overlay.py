...
class ExternalOverlay:
    ...
    def start(self, overlay_delay: float = 0.01):
        self.ui_thread = threading.Thread(target=self._init_ui)
        self.ui_thread.start()
        time.sleep(0.5)
        if(self.target_hwnd == None or self.overlay_hwnd == None):
            raise Exception("Creating handles failed.")
        self.hook_thread = threading.Thread(target=self._hook_to_target(overlay_delay))
    ...
    def _hook_to_target(self, overlay_delay: float):
        ...
        while True:
            try:
                ...
                win32gui.CloseWindow(self.overlay_hwnd)
                dpg.stop_dearpygui()
                #self.ui_thread._stop()
                #self.hook_thread._stop()

    def _init_ui(self):
        dpg.create_context()
        dpg.create_viewport(title=self.overlay_name, width=1000, height=1000, decorated=False, always_on_top=True, clear_color=[0.0,0.0,0.0,0.0]),
        dpg.setup_dearpygui()
        dpg.show_viewport()
